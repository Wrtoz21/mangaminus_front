...
export default function GiveCoin() {
    ...
    const { admin, uploadCoin, userUpdated } = getUser()
    ...
    console.log(admin)
    const giveCoin = async (e) => {
        e.preventDefault()
        try {
            const giveUser = [userSelect, coin]
            await uploadCoin(giveUser).then(() => {
                if (userUpdated) {
    ...
    return <form onSubmit={giveCoin}>
        <div className="flex gap-2 p-3 items-center border-b border-black">
            ...
            <div className="flex  ">
                <button type="submit" className="bg-green-600 rounded-full border p-1">
                    {!loading &&
                        <div onClick={() => {
                    ...
                    {loading && <div
                        ...
                        role="status" className="flex gap-1 w-20">
